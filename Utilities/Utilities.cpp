...
std::string StringHelper::extractFileName(const std::string& s, char c) {
...
std::string StringHelper::getFolderNameFromFile(std::string fileName)
{
	std::size_t found = fileName.find_first_of("_");
	std::string folderName = fileName.substr(found + 1);
	return folderName;
}
